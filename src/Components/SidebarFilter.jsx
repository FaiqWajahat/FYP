...
const FILTERS = {
  ...
  fabrics: ["100% Cotton", "Poly-Cotton Blend", "French Terry", "Nylon / Windbreaker"],
  gsm: ["180-250 (Light)", "280-350 (Mid)", "400+ (Heavyweight)"]
};

const SidebarFilter = ({ mobile, onClose }) => {
  const pathname = usePathname();
  const [selectedCategory, setSelectedCategory] = useState("All Products");

  useEffect(() => {
    // Logic to highlight category based on URL
    ...
    if (!pathSegment || pathSegment === 'shop' || pathSegment === 'all') {
      ...
      setSelectedCategory(match || formattedTitle);
    }
  }, [pathname]);
  ...
  return (
    <div className={`w-full ${mobile ? 'p-6 h-full flex flex-col' : 'w-64 space-y-8 sticky top-32 h-fit hidden lg:block'}`}>
        ...
        <div className="pt-6 border-t border-slate-100 mb-8">
          ...
          <div className="space-y-3">
            {FILTERS.fabrics.map((fabric) => (
              <div key={fabric} className="flex items-center gap-3 cursor-pointer group">
                <div className="w-5 h-5 rounded border border-slate-300 group-hover:border-blue-400 bg-white"></div>
                <span className="text-sm text-slate-600 group-hover:text-blue-600">{fabric}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-slate-100">
          ...
          <div className="space-y-3">
            {FILTERS.gsm.map((weight) => (
              <div key={weight} className="flex items-center gap-3 cursor-pointer group">
                <div className="w-5 h-5 rounded border border-slate-300 group-hover:border-blue-400 bg-white"></div>
                <span className="text-sm text-slate-600 group-hover:text-blue-600">{weight}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Mobile Footer */}
      {mobile && (
        <div className="mt-4 pt-6 border-t border-slate-100 flex gap-4 flex-shrink-0">
          <button className="flex-1 py-3 border border-slate-200 rounded-xl font-bold text-slate-600 hover:bg-slate-50">Reset</button>
          <button onClick={onClose} className="flex-1 py-3 bg-slate-900 text-white rounded-xl font-bold shadow-lg hover:bg-slate-800">Show Results</button>
        </div>
      )}
    </div>
  );
